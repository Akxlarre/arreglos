...
<tbody id="tab_clientes">
	<tr><td colspan="11" align="center"><i class="fa fa-spinner fa-pulse fa-2x fa-fw"></i></td></tr>
</tbody>
</table>
</div>
</div>
...
<div class="row">
	...
	<div class="col-md-6">
		...
		<div class="form-group row">
            <label for="rrut" class="col-sm-4 col-form-label">Tipo Prestador</label>
            <div class="col-sm-8">
                <select name="tipo_prestador" id="tipo_prestador" class="form-control">
...
$.get("operaciones.php", {numero:''+Math.floor(Math.random()*9999999)+'',operacion:'getTabClientes',retornar:'no'},function(data){
console.log(data);
...
x++;
let active = '';
if(parseInt(valor.estadows)==1){
	active = '<label class="switchcustom" for="checkbox_'+index+'">'+
			'<input type="checkbox" id="checkbox_'+index+'" onclick="activarMonitoreo('+index+')" checked/>'+
			'<div class="slidercustom roundcustom"></div>'+
		'</label>';
}
else{
	active = '<label class="switchcustom" for="checkbox_'+index+'">'+
			'<input type="checkbox" id="checkbox_'+index+'" onclick="activarMonitoreo('+index+')" />'+
			'<div class="slidercustom roundcustom"></div>'+
		'</label>';
}
filas+="<tr id='cliente"+index+"'><td>"+x+"</td><td class='font-weight-lighter' nowrap>"+valor.rut+"</td><td class='font-weight-lighter'>"+valor.cuenta+"</td><td class='font-weight-lighter'>"+valor.razonsocial+"</td><!--<td class='font-weight-lighter'>"+valor.giro+"</td>--><td class='font-weight-lighter'>"+valor.tipo_prestador+"</td><td class='oculto' class='font-weight-lighter'>"+valor.direccion+"</td><td class='font-weight-lighter'>"+valor.telefono+"</td><td class='font-weight-lighter'>"+valor.correo+"</td><td class='font-weight-lighter'>"+active+"</td><td class='text-center'><span class='btn btn-sm btn-info btn-sm btn-circle' onclick='verContactos(\""+index+"\");'><i class='fa fa-address-book' aria-hidden='true'></i></span></td><td class='text-center'><span class='btn btn-sm btn-warning btn-sm btn-circle' onclick='EditarCliente(\""+index+"\");'><i class='fa fa-edit' aria-hidden='true'></i></span></td><td class='text-center'><span class='btn btn-sm btn-danger btn-sm btn-circle' onclick='eliminarcliente(\""+index+"\")'><i class='fa fa-trash' aria-hidden='true'></i></span></td></tr>";
});
$("#tab_clientes").html(filas);
...
$("input[name='cuenta']").val(valor.cuenta);
$("input[name='rlegal']").val(valor.rlegal);
$("input[name='rrut']").val(valor.rrut);
...
$("input[name='nombre']").val(valor.nombre);
$("input[name='usuariows']").val(valor.usuariows);
$("input[name='clavews']").val(valor.clavews);
$("input[name='nombrews']").val(valor.nombrews);
$("#tipo_prestador").val(valor.tipo_prestador.toLowerCase());
...
$('#btncrearcliente').removeClass('btn-success').addClass('btn-warning text-white').text('Actualizar Cliente');
$('#formoperacion').val('editarcliente');
$('#myTab a[href="#crearcliente"]').tab('show');
}
});
}
