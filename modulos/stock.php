    ...
    <div class="card">
        <div class="card-header p-2">
            <ul class="nav nav-pills" id="myTab">
                ...
                <li class="nav-item"><a onclick="getBodegasGps()" id="navlistguia" class="nav-link" href="#bodegasgps" data-toggle="tab">Bodegas</a></li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content">
                ...
                <div class="tab-pane" id="bodegasgps">
                    <table class="table table-bordered table-sm" id="tbl_bodegas">
                        <thead class="thead-dark">
                        <tr>
                            <th scope="col">N°</th>
                            <th scope="col" >GPS</th>
                            ...
                            <th scope="col" nowrap>Condicion</th>
                            <th scope="col" nowrap>Bodega</th>
                            <th scope="col" nowrap></th>
                        </tr>
                        </thead>
...
<script>
        ...
        $.get("operaciones.php", {numero:''+Math.floor(Math.random()*9999999)+'',operacion:'newgetInventarioxProducto',id:idpro,retornar:'no'},function(data){
            ...
            $.each(inv, function(i, item){
                ...
                if(i==0){
                    ...
                    prodcutosbodega.push({'nombre':(item.cargo==26?'Bodega Principal':item.bodega),'cantidad':1})
                    ...
                    let bodeg = prodcutosbodega.filter(bod => bod.nombre == (item.cargo==26?'Bodega Principal':item.bodega))
                    if (bodeg.length > 0) {
                        var cantnue = bodeg[0].cantidad+1;
                        bodeg[0].cantidad = cantnue;
                    }else{
                        prodcutosbodega.push({'nombre':(item.cargo==26?'Bodega Principal':item.bodega),'cantidad':1})
                    }
            ...
            var nuehtml = '';
            $.each(prodcutosbodega, function(i, item){
                nuehtml+='<tr><td>'+item.nombre+'</td><td>:'+item.cantidad+'</td></tr>';
            })
            $("#paratotalesbodegas").html('<table><tr><td>Bodega</td><td>Cantidad</td></tr>'+nuehtml+'</table>');
        });
    }
    ...
    */
    ...
    ////////////////////////// FUNCIONES DE BODEGAS GPS //////////////////////////////////
    let listBodega = [];
    function getBodegasGps(){
        //$('#tbl_bodegas tbody').html('');
        ...
        $('#tbl_bodegas').DataTable({
            ajax: 'operaciones.php?numero='+Math.floor(Math.random()*9999999)+'&operacion=listarBodega&retornar=no',
            processing: true,
            serverSide: true,
            "language": {
            ...
            columns: [
                ...
                { data: 'gps',"defaultContent": "" },
                { data: 'ser_codigo',"defaultContent": "" },
        ...
        $.get("operaciones.php", {numero:''+Math.floor(Math.random()*9999999)+'',operacion:'listarBodega',retornar:'no'},function(data){
            if(data!='' && data!=null){
                data = $.parseJSON(data);
                if(data.data.length>0){
                    listBodega = data.data;
                    let fila = '';
                    $('.tool').tooltip('dispose')
                    $('#tbl_bodegas tbody').html('');
                    $.each(data.data,function(i,item){
                        let num = $('#tbl_bodegas tbody tr').length;
                        ...
                        fila += '<tr id="fila_'+i+'">' +
                            '<td>'+(i+1)+'</td><td>'+item.gps+'</td>' +
                            '<td>'+item.seriegps+'</td>' +
                            '<td>'+item.accesorio+'</td>' +
                            '<td>'+item.serieaccesorio+'</td>' +
                            '<td>'+item.bodega+'</td>'+sensores+'</tr>';
                    });
                    $('#tbl_bodegas tbody').append(fila);
                }
            }
