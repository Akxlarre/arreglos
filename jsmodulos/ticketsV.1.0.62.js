...
$(document).ready(function() {
    ...
    console.log( "delete ticket : ",deleteTicket );
...
function dosFunciones(){
    ...
    $('#tck_asig_cli').chosen({width:'100%'});
    $('#tck_asig').chosen({width:'100%'});
}
    ...
    $.ajax({
        ...
        beforeSend: function(respuesta) {
            ...
            $.each(data, function(i, items) {
                ...
                if(items.CliAsig== null){
                    items.CliAsig = "Sin Asignar";
                 }
                if(items.Cuenta== 'cloux'){
                    items.Cuenta = "DS";
                 }
                if(items.Tipo==1){
                   tipoVar = "Incidencia";
                ...
                if(items.Tipo==2){
                 ...
                 if(items.Estado_proceso==2){
                  ...
                  if(items.Asignado == null){
                    ...
                    /*if(items. Estado_proceso!=4 && items.Estado_proceso!=5){
                    fila += `<tr id="trtick" name="trtick" for="trtick" class="clickableRow" style="cursor:pointer; background-color:` + colorClase + ` " 
                    onclick="editarTicket(` + items.id + `,`+items.Estado_proceso+`); listarComentarios(` + items.id + `);listarTiempos(` + items.id + `)">
                
                <td id="tdtick" name="tdtick" for="tdtick">`+items.id+`</td>
                <td id="tdtick" name="tdtick" for="tdtick">`+items.Cuenta+`</td>
                <td id="tdtick" name="tdtick" for="tdtick">`+items.CliAsig+`</td>
                ...
                <td  onclick="event.cancelBubble=true; return false;"><button data-toggle="tooltip" data-placement="left" title="Borrar Usuario Jaime 1" type="button" onclick="borrarTicket( `+ items.id + `,`+ items.Estado_proceso + `)" class="btn  btn-sm tools" style="color:white;background-color:` + colorroj + `"><i class="fa fa-trash"></i></td>
                </tr>`;
                    } */
                ...
                if(items.Estado_proceso!=4 && items.Estado_proceso!=5){
                    ...
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.id+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.Cuenta+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.CliAsig+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.usuario+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+asignado+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.Fecha_Hora+`</td>`;              
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+tipoVar+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.Asunto+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.Descripcion+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+proceso+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.Tiempo_transcurrido+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+items.Fecha_Actualiza+`</td>`;
                    fila += `<td id="tdtick" name="tdtick" for="tdtick">`+inter+`</td>`;
                    if( deleteTicket == 1 ){
                        fila += `<td  onclick="event.cancelBubble=true; return false;"><button data-toggle="tooltip" data-placement="left" 
                        title="Borrar Usuario" type="button" onclick="borrarTicket( `+ items.id + `,`+ items.Estado_proceso + `)" 
                        class="btn  btn-sm tools" style="color:white;background-color:` + colorroj + `"><i class="fa fa-trash"></i></td>`; 
                    }
                    fila += `</tr>`;
                }
...
if(Estado_proceso == 5){
        Swal.fire(
            {
                title: "!LO SENTIMOS!",
                text: "Este ticket ya ah sido anulado",
                icon: "info",
...
if(Estado_proceso == 1 || Estado_proceso == 4){
    Swal.fire({
        title: "¿Seguro desea anular el ticket?",
        text: "Este registro quedara nulo",
        icon: "info",
...
else if(Estado_proceso == 2 || Estado_proceso == 3){
    Swal.fire(
        {
            title: "!LO SENTIMOS!",
...
function eliminarTicket(id){
    $.get("../cloux/Modelo/operaciones_tickets.php", { numero: '' + Math.floor(Math.random() * 9999999) + '', operacion: 'eliminarTicket', id: id, retornar: 'no' }, function(data) {
        if (data != null && data != '') {
            ...
            if (data.status == 'OK') {
                listarTickets();
                Swal.fire('Exito al anular ticket.', '', 'success')
                setTimeout(location.reload(), 3000);
            } else {
                Swal.fire('Error al anular ticket, intente nuevamente.', '', 'error')
            }
        }
    });
}
...
function editarTicket(id){
    ...
    let ticket = tickets.filter(tck => parseInt(tck.id) == parseInt(id));   
      if(ticket.length > 0){
        ...
        $('#tck_asig_cli').val(ticket[0].CliAsig);
        $('#tck_asig_cli').chosen({width:'100%'});
        ...
        if(ticket[0].Estado_proceso == 1){
           // $('#tomarTickets').html('<i class="fas fa-check">Tomar</i>').prop('style','background-color: yellow').attr('onclick','confirmarticket()')
            $('#btnconftck').html('<button type="button" class="btn btn-sm float-right" name="tomarTickets" for="tomarTickets" id="tomarTickets" data-toggle="modal" data-target="#tomarTickets" onclick="confirmarticket()" title="Tomar tickets" style=";margin-right:1px;margin-top:1px;background-color: yellow"><i class="fas fa-check">Tomar</i></button>')
        ...
        if(ticket[0].Estado_proceso == 2){
            ...
            $('#btnconftck').html('<button type="button" class="btn btn-sm float-right" name="tomarTickets" for="tomarTickets" id="tomarTickets" data-toggle="modal" data-target="#tomarTickets" onclick="finalizarTicket()" title="Tomar tickets" style=";margin-right:1px;margin-top:1px;background-color: orange"><i class="fas fa-check">Enviar</i></button>')
        ...
        if(ticket[0].Estado_proceso == 3){
            ...
            $('#btnconftck').html('<button type="button" class="btn btn-sm float-right" name="tomarTickets" for="tomarTickets" id="tomarTickets" data-toggle="modal" data-target="#tomarTickets" onclick="conFinticket('+id+')" title="Tomar tickets" style=";margin-right:1px;margin-top:1px;background-color: #82d2bf"><i class="fas fa-check">Finalizar</i></button>');
            ...
            $('#btnconftck').html('<button type="button" class="btn btn-sm float-right" name="tomarTickets" for="tomarTickets" id="tomarTickets" data-toggle="modal" data-target="#tomarTickets"  title="Tomar tickets" style=";margin-right:1px;margin-top:1px;background-color: #82d2bf"><i class="fas fa-check">Finalizado</i></button>');

            $('#btncreartck').html('Actualizar Ticket').prop('disabled',true);
            $('#tomarTickets').prop('disabled',true);
            $('#tck_comentario').prop('disabled',true);
        }
        if(ticket[0].Estado_proceso == 5){
            $('#tck_comentario').prop('disabled',true);
            $('#tck_tipo').prop('disabled',true);
            $('#tck_descripcion').prop('disabled',true);
            $('#tck_asig').prop('disabled',true);
            $('#btncreartck').html('Actualizar Ticket').prop('disabled',true);
            $('#tck_asig_cli').prop('disabled',true);
        }
    }
...
function conFinticket(id){
    if(id != null){
        
        Swal.fire({
            title: "¿Confirmar Finalización ticket?",
            ...
            if (result.isConfirmed) {
                ...
                ;
                $.get("../cloux/Modelo/operaciones_tickets.php", { numero: '' + Math.floor(Math.random() * 9999999) + '', operacion: 'conticket', id: id, retornar: 'no' }, function(data) {
                    if (data != null && data != '') {
                        data = $.parseJSON(data);
                        if (data.status == 'OK') {
                            //$('#btnrechftck').prop('disabled',true);
